<template>
	<div class="sy-app-container">
		<div class="sy-home-header">
			<sy-header :title="$store.state.lang.base.title" :name="userName" :versionList="versionList" 
                :lang="changeLang" @changeLange="handleChangeLange" :lang-list="langList"></sy-header>
		</div>
...
import { getUnitLicense ,getUnitCodeConfigLang } from './request.js';
...
export default {
	...
	beforeRouteEnter(to, from, next) {
		getUnitCodeConfigLang('sl', store.state.unitId).then(res=>{
			let arr = [];
			res.forEach((_a,a)=>{
				arr.push({
					value: _a.value,
	...
	methods: {
		init(){
			this.userName = this.$store.state.realName || '';
      		this.changeLang = window.localStorage.getItem("lange_") || "zh-CN";
		...
		// 查询到期时间
		getLimitTime(){
			getUnitLicense('sl',this.$store.state.unitId).then(res=>{
