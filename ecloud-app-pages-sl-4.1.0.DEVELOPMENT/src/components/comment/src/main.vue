  ...
  created() {
    ...
    if (userMsg && userMsg.data) {
      ...
      if (this.$store.state.userId) {
        ...
        this.avatar =
          this.baseUrl + "/api/storage/show/avatar/" + this.$store.state.userId;
      } else {
        this.userId = "";
    ...
    bus.$on("reply-add", val => {
      ...
      this.hotCommentDataLoad.forEach((value, index) => {
        if (value.id === val.commentId) {
          value.replys.push(obj2);
        }
      });
      ...
      getCommentByTime({
        ...
        if (
          ...
          resNew.data.list.forEach((valNew, indexNew) => {
            valNew.datetime = moment(valNew.datetime)
              .startOf("minute")
              .fromNow();
      ...
      getHotComment({
        ...
        if (
          ...
          resHot.data.list.forEach((valHot, indexHot) => {
            valHot.datetime = moment(valHot.datetime)
              .startOf("second")
              .fromNow();
