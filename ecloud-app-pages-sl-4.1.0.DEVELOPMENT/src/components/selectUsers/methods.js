        ...
        isArry(o) {
            ...
            searchUser() {
                this.userJson = _.filter(this.userJsonBack || [], (o) => {
                    if (this.ignore.length > 0) {
                        return ((o.loginName && o.loginName.indexOf(this.keyWord) != -1) || (o.realName && o.realName.indexOf(this.keyWord) != -1)) && this.ignore.indexOf(o.id) == -1;
                    }
            ...
            createGroupTypes() {
                ...
                if (this.groups.length == 0) {
                    if (this.selectUserType == 2) {
                        this.tabs.push({
                            id: 'all',
                            name: this.textObj.all,
                            type: 'all'
                        });
                        this.tabs.push({
                            id: 'department',
                            name: this.textObj.department,
                            type: 'department'
                        });
                        this.tabs.push({
                            id: 'subject',
                            name: this.textObj.subject,
                            type: 'subject'
                        });
                    }
                    this.tabs.push({
                        id: 'gradeclass',
                        name: this.textObj.gradeclass,
                        type: 'gradeclass'
                    });
                    /*if (this.selectUserType == 2) {
                        this.tabs.push({
            ...
            createUserGroups() {
                findUserGroups({
                    ...
                    this.oldTreeDatas = _.clone(data.data);
                    this.parentIds = [];
                    this.childrenIds = [];
                    this.treeDatas = [];
                    this.treeDatasBack = [];
            ...
            okHandle() {
                let _users = [];
                this.checkedUserJson.forEach((_d, d) => {
                    let _clazz = _.find(this.oldTreeDatas, {
                        id: _d.classId
                    });

                    if (_clazz) {
                        if (_clazz.type == 'class') {
                            _d.className = _clazz.name
                        }
                        if (_clazz.parentId) {
                            let obj = _.find(this.oldTreeDatas, {
                                id: _clazz.parentId
                            });
                            _d.gardeName = obj ? obj.name : '';
                            _d.gardeId = obj ? obj.id : '';
                        }
                    }
                    _users.push(_d);
                })
                this.callback(_users, this);
            },
            handleCheckAll(e) {
                var _this = this;
                if (e.target.checked) {
            ...
            gradeClassTreeChange(o, node) {
                // console.log('年级班级树', o, node)
                this.groupId = o.id;
